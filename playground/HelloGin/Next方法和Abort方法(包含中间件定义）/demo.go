...
func main() {
	...
	/*Next方法是用在中间件中的，它的作用就是：如果有两个中间件按顺序前后注册的话，在第一个中间件程序的代码段中，Next方法出现在哪一行代码，那一行代码就执行后面的中间件程序(或接口handler函数)，
	后面的中间件程序执行好之后再执行第一个中间件程序中Next方法出现的那一行代码后面的代码段。它的作用就是这么这样*/
	/*Abort方法也是用在中间件中，只不过它的作用是：如果有两个中间件按顺序前后注册的话，在第一个中间件程序的代码段中，只要Abort方法出现，那么后面注册的那个中间件程序(或接口handler函数)就不会执行。*/
	...
	//输出结果是：handler已执行
	r.GET("/m7", middleware7(), func(c *gin.Context) {
		c.String(200, "handler已执行")
	})

	//输出结果为空
	r.GET("/m8", middleware8(), func(c *gin.Context) {
		c.String(200, "handler已执行")
	})
...
func middleware7() gin.HandlerFunc {
	return func(c *gin.Context) {
		c.Next()
	}
}

func middleware8() gin.HandlerFunc {
	return func(c *gin.Context) {
		c.Abort()
	}
}
